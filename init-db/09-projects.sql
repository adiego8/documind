-- AssistantJS Projects System (EmailJS-style for frontend integration)
-- Public project IDs with session-based authentication for safe frontend use

\c ragdb;

-- Create projects table for frontend integrations
...
-- Create public_sessions table for short-lived authentication
...
-- Create session_usage table for tracking requests and rate limiting
...
-- Function to check project rate limits
...
-- Function to record session usage
...
SELECT 'Frontend-safe project IDs with session-based authentication enabled' AS info;